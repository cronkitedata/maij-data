...
1. Using the if_else() function, create a column that shows whether or not the loan was forgiven. (None have been charged off yet)
...
4.  The [list of the first digits of the industry code](https://www.census.gov/naics/?58967?yearbck=2017) can help you determine what kind of business it was. Use a mutate statement using str_like() and case_when() to distinguish between a) retail trade (this includes restaurants and stores), b) Mining and agriculture, and c) Professional services. Code everything as as "Other". This might be hard, and it will require str_like, an "OR" condition, and the case_when function.  Get as far as you can with this. 
